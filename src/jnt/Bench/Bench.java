...
public class Bench implements Runnable {
    ...
    void parseDescriptor(InputStream stream) throws IOException {
        ...
        while (in.nextToken() == StreamTokenizer.TT_WORD) {
            ...
            } else if (var.equals("target")) {
                targetClass = parseString(in, "target");
            } else if (var.equals("submission_email")) {
                subemail = parseString(in, "submission_email");
            } else if (var.equals("submission_url")) {
                suburl = parseString(in, "submission_url");
            } else if (var.equals("units")) {
                units = parseString(in, "units");
            } else if (var.equals("decimals")) {
                decimals = (int) parseNumber(in, "decimals");
            } else if (var.equals("segments")) {
                parseSegments(in);
            } else if (var.equals("entries")) {
                parseEntries(in);
            } else {
                throw new IOException("Unknown parameter " + var);
            }
        }
        checkDefaultSegment();
    }
        ...
        while ((tok = in.nextToken()) != ')') {
            ...
            if ((tok == StreamTokenizer.TT_WORD) || (tok == '"')) {
                ...
                while ((tok = in.nextToken()) == StreamTokenizer.TT_WORD) {
                    ...
                    getPunct(in, '=');
                    if (var.equals("units")) {
                        u = parseString(in, "units");
                    } else if (var.equals("decimals")) {
                        d = (int) parseNumber(in, "decimals");
                    } else if (var.equals("shortname")) {
                        s = parseString(in, "shortname");
                    }
                }
                if (tok != ')') {
                    parseError(in, ")", "char");
    ...
    public boolean isSubmittable() {
        return (subemail != null) || (suburl != null);
    }
    ...
    /**
     * Return the measured values for all entries in segment seg in this
     * benchmark.
     */
    ...
    /**
     * Return the measured values for the current measurement (if any) of this
     * benchmark.
     */
    ...
    /**
     * Start the timer for segment. It resets the timer for this segment if any
     * previous measurement had been made. This is intended to be used as a
    ...
    /**
     * Stop the timer for this segment. It returns the timing, up to this point.
    ...
    /**
     * Reset the timer for this segment. This is intended to be used as a
     * callback from Target.
     */
    ...
    /**
     * Resume the timer for segment. This is like starting the timer without
     * resetting it first. This is intended to be used as a callback from
     * Target.
     */
    ...
    void insertEntry(double values[]) {
        ...
        if (ascending) {
            ...
            {
                if (v0 < ((Entry) entries.elementAt(specpos)).values[seg]) {
                    break;
                }
            }
        } else {
    ...
    public void printMeasurements(PrintStream out) {
        ...
        int m = 0;
        for (int i = 0; i < segments.length; i++) {
            if (segments[i].decimals > m) {
                m = segments[i].decimals;
            }
        }
...
class Entry {
    ...
    /**
     * Create an Benchmark Entry describing measurements made on a particular
     * platform.
     */
