...
/**
 * Plotter for the Bench Package Draws a horizontal bar graph, with labels for
 * each bar at the left. A `Special' row, in a different color, is used to
 * highlight the measurements of the current system.
 *
...
public class Plotter extends Canvas {
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < n; i++) {
            if ((tmp = metrics.stringWidth((String) labels[i])) > labLen) {
                labLen = tmp;
            }
            if (values[i] > max) {
                max = values[i];
