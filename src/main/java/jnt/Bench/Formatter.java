...
/**
 * An EXTREMELY Lame & Minimal Number formatter. Only what is essential for the
 * Benchmarker... I'll evolve it if necessary.
 *
...
public class Formatter {
    ...
    public static void addColumn(String table[], String column[], int justify) {
        ...
        for (int i = 0; i < table.length; i++) {
            if ((ll = table[i].length()) > l0) {
                l0 = ll;
            }
            if ((ll = column[i].length()) > l1) {
                l1 = ll;
            }
        }
        for (int i = 0; i < table.length; i++) {
