...
/**
 * SubmitDialog provides a Dialog box for filling in a benchmark submission. It
 * extracts the relevent information from System.Properties of the VM, and
 * provides text boxes for filling in other information. It uses Bench.SendMail
 * to send the message, and uses callbacks to the Applet (if any) to inform of
...
public class SubmitDialog extends Frame {
    ...
    /**
     * Create a SubmitDialog to report on a new measurement of the Benchmark
     * Target described in bench.
     */
    ...
    void doSend() {
        ...
        try {
            ...
            if (recipient != null) {
                try {
                    server = applet.getCodeBase().getHost();
                } catch (Exception e) {
                }
                if (server.length() == 0) {
